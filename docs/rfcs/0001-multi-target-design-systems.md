...
| `--size-selector` / `--size-field` | Kept verbatim (already in the `size` category). The lynx `FIELD_STEPS`/`SELECTOR_STEPS` multiplication tables are deleted; recipes write `calc(var(--size-field) * 12)` and the lynx target folds it (§3.3). | |
