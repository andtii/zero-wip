// carbon — generated by @sigx/zero-kit. Do not edit.
//
// Augments @sigx/zero's ZeroVocabulary with this design system's compiled
// vocabulary. Opt in with:  import '@sigx/<ds>/register';
declare module '@sigx/zero' {
    interface ZeroVocabulary {
        theme: 'white' | 'g100';
        breakpoint: 'sm' | 'md' | 'lg';
        property: '--border' | '--carbon-danger' | '--carbon-danger-ink' | '--carbon-focus' | '--carbon-interactive' | '--carbon-interactive-ink' | '--carbon-line' | '--carbon-secondary' | '--carbon-secondary-ink' | '--carbon-toggle-on' | '--color-base-100' | '--color-base-200' | '--color-base-300' | '--color-base-content' | '--disabled-opacity' | '--duration-fast' | '--duration-instant' | '--duration-normal' | '--duration-slow' | '--ease-accelerate' | '--ease-decelerate' | '--ease-linear' | '--ease-standard' | '--font-mono' | '--font-sans' | '--leading-none' | '--leading-normal' | '--leading-tight' | '--radius-box' | '--radius-field' | '--radius-selector' | '--shadow-lg' | '--shadow-md' | '--shadow-sm' | '--shadow-xl' | '--shadow-xs' | '--size-field' | '--size-selector' | '--space-2xl' | '--space-2xs' | '--space-lg' | '--space-md' | '--space-sm' | '--space-xl' | '--space-xs' | '--text-2xl' | '--text-3xl' | '--text-fixed-2xl' | '--text-fixed-3xl' | '--text-fixed-lg' | '--text-fixed-md' | '--text-fixed-sm' | '--text-fixed-xl' | '--text-fixed-xs' | '--text-lg' | '--text-md' | '--text-sm' | '--text-xl' | '--text-xs' | '--tracking-normal' | '--tracking-tight' | '--tracking-wide' | '--weight-bold' | '--weight-medium' | '--weight-normal' | '--weight-semibold';
        tokens: {
            radius: 'selector' | 'field' | 'box';
            size: 'selector' | 'field';
            font: 'sans' | 'serif' | 'mono' | 'display';
            text: 'xs' | 'sm' | 'md' | 'lg' | 'xl' | '2xl' | '3xl';
            weight: 'normal' | 'medium' | 'semibold' | 'bold';
            leading: 'none' | 'tight' | 'normal' | 'relaxed';
            tracking: 'tight' | 'normal' | 'wide';
            space: '2xs' | 'xs' | 'sm' | 'md' | 'lg' | 'xl' | '2xl';
            shadow: 'xs' | 'sm' | 'md' | 'lg' | 'xl';
            duration: 'instant' | 'fast' | 'normal' | 'slow';
            ease: 'linear' | 'standard' | 'emphasized' | 'accelerate' | 'decelerate';
        };
        components: {
            /** tabs — size wired. */
            'tabs': {
                /** Accepts `color` at runtime, but carbon declares no color axis at all — the attribute would match nothing. */
                color: never;
                size: 'sm' | 'md' | 'lg';
                /** Accepts `variant` at runtime, but no carbon recipe wires it — the attribute would match nothing. */
                variant: never;
                axes: Record<string, never>;
                mods: Record<string, never>;
            };
            /** collapsible — no axis wired by carbon; every axis errors under this register module. */
            'collapsible': {
                /** Accepts `color` at runtime, but carbon declares no color axis at all — the attribute would match nothing. */
                color: never;
                /** Accepts `size` at runtime, but no carbon recipe wires it — the attribute would match nothing. */
                size: never;
                /** Accepts `variant` at runtime, but no carbon recipe wires it — the attribute would match nothing. */
                variant: never;
                axes: Record<string, never>;
                mods: Record<string, never>;
            };
            /** switch — size wired. */
            'switch': {
                /** Accepts `color` at runtime, but carbon declares no color axis at all — the attribute would match nothing. */
                color: never;
                size: 'sm' | 'md' | 'lg';
                /** Accepts `variant` at runtime, but no carbon recipe wires it — the attribute would match nothing. */
                variant: never;
                axes: Record<string, never>;
                mods: Record<string, never>;
            };
            /** dialog — no axis wired by carbon; every axis errors under this register module. */
            'dialog': {
                /** Accepts `color` at runtime, but carbon declares no color axis at all — the attribute would match nothing. */
                color: never;
                /** Accepts `size` at runtime, but no carbon recipe wires it — the attribute would match nothing. */
                size: never;
                /** Accepts `variant` at runtime, but no carbon recipe wires it — the attribute would match nothing. */
                variant: never;
                axes: Record<string, never>;
                mods: Record<string, never>;
            };
            /** popover — no axis wired by carbon; every axis errors under this register module. */
            'popover': {
                /** Accepts `color` at runtime, but carbon declares no color axis at all — the attribute would match nothing. */
                color: never;
                /** Accepts `size` at runtime, but no carbon recipe wires it — the attribute would match nothing. */
                size: never;
                /** Accepts `variant` at runtime, but no carbon recipe wires it — the attribute would match nothing. */
                variant: never;
                axes: Record<string, never>;
                mods: Record<string, never>;
            };
            /** tooltip — no axis wired by carbon; every axis errors under this register module. */
            'tooltip': {
                /** Accepts `color` at runtime, but carbon declares no color axis at all — the attribute would match nothing. */
                color: never;
                /** Accepts `size` at runtime, but no carbon recipe wires it — the attribute would match nothing. */
                size: never;
                /** Accepts `variant` at runtime, but no carbon recipe wires it — the attribute would match nothing. */
                variant: never;
                axes: Record<string, never>;
                mods: Record<string, never>;
            };
            /** menu — no axis wired by carbon; every axis errors under this register module. */
            'menu': {
                /** Accepts `color` at runtime, but carbon declares no color axis at all — the attribute would match nothing. */
                color: never;
                /** Accepts `size` at runtime, but no carbon recipe wires it — the attribute would match nothing. */
                size: never;
                /** Accepts `variant` at runtime, but no carbon recipe wires it — the attribute would match nothing. */
                variant: never;
                axes: Record<string, never>;
                mods: Record<string, never>;
            };
            /** field — no axis wired by carbon; every axis errors under this register module. */
            'field': {
                /** Accepts `color` at runtime, but carbon declares no color axis at all — the attribute would match nothing. */
                color: never;
                /** Accepts `size` at runtime, but no carbon recipe wires it — the attribute would match nothing. */
                size: never;
                /** Accepts `variant` at runtime, but no carbon recipe wires it — the attribute would match nothing. */
                variant: never;
                axes: Record<string, never>;
                mods: Record<string, never>;
            };
            /** checkbox — size wired. */
            'checkbox': {
                /** Accepts `color` at runtime, but carbon declares no color axis at all — the attribute would match nothing. */
                color: never;
                size: 'sm' | 'md' | 'lg';
                /** Accepts `variant` at runtime, but no carbon recipe wires it — the attribute would match nothing. */
                variant: never;
                axes: Record<string, never>;
                mods: Record<string, never>;
            };
            /** radio-group — size wired. */
            'radio-group': {
                /** Accepts `color` at runtime, but carbon declares no color axis at all — the attribute would match nothing. */
                color: never;
                size: 'sm' | 'md' | 'lg';
                /** Accepts `variant` at runtime, but no carbon recipe wires it — the attribute would match nothing. */
                variant: never;
                axes: Record<string, never>;
                mods: Record<string, never>;
            };
            /** progress — size wired. */
            'progress': {
                /** Accepts `color` at runtime, but carbon declares no color axis at all — the attribute would match nothing. */
                color: never;
                size: 'sm' | 'md' | 'lg';
                /** Accepts `variant` at runtime, but no carbon recipe wires it — the attribute would match nothing. */
                variant: never;
                axes: Record<string, never>;
                mods: Record<string, never>;
            };
            /** slider — size wired. */
            'slider': {
                /** Accepts `color` at runtime, but carbon declares no color axis at all — the attribute would match nothing. */
                color: never;
                size: 'sm' | 'md' | 'lg';
                /** Accepts `variant` at runtime, but no carbon recipe wires it — the attribute would match nothing. */
                variant: never;
                axes: Record<string, never>;
                mods: Record<string, never>;
            };
            /** accordion — no axis wired by carbon; every axis errors under this register module. */
            'accordion': {
                /** Accepts `color` at runtime, but carbon declares no color axis at all — the attribute would match nothing. */
                color: never;
                /** Accepts `size` at runtime, but no carbon recipe wires it — the attribute would match nothing. */
                size: never;
                /** Accepts `variant` at runtime, but no carbon recipe wires it — the attribute would match nothing. */
                variant: never;
                axes: Record<string, never>;
                mods: Record<string, never>;
            };
            /** select — size wired. */
            'select': {
                /** Accepts `color` at runtime, but carbon declares no color axis at all — the attribute would match nothing. */
                color: never;
                size: 'sm' | 'md' | 'lg';
                /** Accepts `variant` at runtime, but no carbon recipe wires it — the attribute would match nothing. */
                variant: never;
                axes: Record<string, never>;
                mods: Record<string, never>;
            };
            /** button — size, variant, mods.expressive, mods.icon-only wired. */
            'button': {
                /** Accepts `color` at runtime, but carbon declares no color axis at all — the attribute would match nothing. */
                color: never;
                size: 'sm' | 'md' | 'lg' | 'xl' | '2xl';
                variant: 'primary' | 'secondary' | 'tertiary' | 'ghost' | 'danger' | 'danger-tertiary' | 'danger-ghost';
                axes: Record<string, never>;
                mods: { 'expressive': boolean; 'icon-only': boolean };
            };
            /** avatar — size wired. */
            'avatar': {
                /** Accepts `color` at runtime, but carbon declares no color axis at all — the attribute would match nothing. */
                color: never;
                size: 'sm' | 'md' | 'lg';
                /** Accepts `variant` at runtime, but no carbon recipe wires it — the attribute would match nothing. */
                variant: never;
                axes: Record<string, never>;
                mods: Record<string, never>;
            };
            /** toast — no axis wired by carbon; every axis errors under this register module. */
            'toast': {
                /** Accepts `color` at runtime, but carbon declares no color axis at all — the attribute would match nothing. */
                color: never;
                /** Accepts `size` at runtime, but no carbon recipe wires it — the attribute would match nothing. */
                size: never;
                /** Accepts `variant` at runtime, but no carbon recipe wires it — the attribute would match nothing. */
                variant: never;
                axes: Record<string, never>;
                mods: Record<string, never>;
            };
            /** combobox — size wired. */
            'combobox': {
                /** Accepts `color` at runtime, but carbon declares no color axis at all — the attribute would match nothing. */
                color: never;
                size: 'sm' | 'md' | 'lg';
                /** Accepts `variant` at runtime, but no carbon recipe wires it — the attribute would match nothing. */
                variant: never;
                axes: Record<string, never>;
                mods: Record<string, never>;
            };
            /** toggle — size wired. */
            'toggle': {
                /** Accepts `color` at runtime, but carbon declares no color axis at all — the attribute would match nothing. */
                color: never;
                size: 'sm' | 'md' | 'lg';
                /** Accepts `variant` at runtime, but no carbon recipe wires it — the attribute would match nothing. */
                variant: never;
                axes: Record<string, never>;
                mods: Record<string, never>;
            };
            /** toggle-group — size wired. */
            'toggle-group': {
                /** Accepts `color` at runtime, but carbon declares no color axis at all — the attribute would match nothing. */
                color: never;
                size: 'sm' | 'md' | 'lg';
                /** Accepts `variant` at runtime, but no carbon recipe wires it — the attribute would match nothing. */
                variant: never;
                axes: Record<string, never>;
                mods: Record<string, never>;
            };
            /** number-input — size wired. */
            'number-input': {
                /** Accepts `color` at runtime, but carbon declares no color axis at all — the attribute would match nothing. */
                color: never;
                size: 'sm' | 'md' | 'lg';
                /** Accepts `variant` at runtime, but no carbon recipe wires it — the attribute would match nothing. */
                variant: never;
                axes: Record<string, never>;
                mods: Record<string, never>;
            };
            /** rating-group — size wired. */
            'rating-group': {
                /** Accepts `color` at runtime, but carbon declares no color axis at all — the attribute would match nothing. */
                color: never;
                size: 'sm' | 'md' | 'lg';
                /** Accepts `variant` at runtime, but no carbon recipe wires it — the attribute would match nothing. */
                variant: never;
                axes: Record<string, never>;
                mods: Record<string, never>;
            };
            /** tree-view — size wired. */
            'tree-view': {
                /** Accepts `color` at runtime, but carbon declares no color axis at all — the attribute would match nothing. */
                color: never;
                size: 'sm' | 'md' | 'lg';
                /** Accepts `variant` at runtime, but no carbon recipe wires it — the attribute would match nothing. */
                variant: never;
                axes: Record<string, never>;
                mods: Record<string, never>;
            };
        };
    }
}

// Fails to compile if a scope above is not in zero's anatomy registry
// (a typo or a version skew would otherwise silently take the open
// fallback — RFC 0002 §3.1).
type _MustBeTrue<T extends true> = T;
type _ScopesValid = _MustBeTrue<
    keyof import('@sigx/zero').ZeroVocabulary['components'] extends import('@sigx/zero').ZeroScope
        ? true : false
>;
export {};
